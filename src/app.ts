...
class MobileMapperApp {
  ...
  setupEventListeners() {
    ...
    document.getElementById("addCircleBtn")!.addEventListener("click", () => this.setTool("circle"));
    ...
    document.getElementById("warpToggle")!.addEventListener("change", (e) => this.toggleWarpMode((e.target as HTMLInputElement).checked));
    ...
    const bindAudioSlider = (id: string, param: keyof AudioSettings) => {
        ...
        slider.addEventListener("input", (e) => {
            if (this.selectedPolygon) {
                (this.selectedPolygon.audioSettings as any)[param] = parseFloat((e.target as HTMLInputElement).value);
            }
        });
    };
    ...
    document.getElementById("addEffectBtn")!.addEventListener("click", () => {
        const type = (document.getElementById("effectTypeSelect") as HTMLSelectElement).value;
        this.addEffect(type);
    });
    ...
    document.querySelectorAll(".arrow-btn").forEach((btn) => {
      btn.addEventListener("click", () => this.finetuneVertex((btn as HTMLElement).dataset.dir!));
    });
    
    document.getElementById("toggleCurveBtn")?.addEventListener("click", () => this.toggleVertexCurve());
  ...
  renderLayersList() {
      ...
      this.polygons.forEach((poly, index) => {
          ...
          item.style.backgroundColor = poly.selected ? "rgba(0,255,157,0.2)" : "transparent";
  ...
  renderEffectsList(poly: Polygon) {
    ...
    if (!poly.effects || poly.effects.length === 0) {
        list.innerHTML = "<div style='opacity:0.5; font-size:12px; padding:8px;'>No effects added</div>";
        return;
    }
    ...
    poly.effects.forEach(effect => {
        ...
        if (['brightness', 'contrast', 'saturation', 'hue'].includes(effect.type)) {
            const min = effect.type === 'brightness' ? -1 : 0;
            const max = effect.type === 'brightness' ? 1 : 2;
            const step = effect.type === 'hue' ? 0.01 : 0.1;
            controls = `
                <div class="control-group">
                    <label>Value: <span id="val-${effect.id}">${p.value.toFixed(2)}</span></label>
            ...
            controls = `
                ...
                <div class="control-group">
                    ...
                    <input type="range" min="0" max="1" step="0.1" value="${p.intensity}"
                           data-effect-id="${effect.id}" data-param="intensity">
                </div>
                <div class="control-group">
                    <label>Anim Speed</label>
                    <input type="range" min="0" max="5" step="0.1" value="${p.speed !== undefined ? p.speed : 1.0}"
            ...
            `;
             ...
             controls = `
                <div class="control-group">
                    <label>Width</label>
                    <input type="range" min="0.01" max="0.2" step="0.01" value="${p.width}"
                           data-effect-id="${effect.id}" data-param="width">
                </div>
                <div class="control-group">
                    <label>Pulse Speed</label>
                    <input type="range" min="0" max="10" step="0.1" value="${p.speed !== undefined ? p.speed : 0.0}"
                           data-effect-id="${effect.id}" data-param="speed">
                </div>
            `;
        }
  ...
  startControlsDrag(e: MouseEvent) {
    ...
    const clientY = (e as any).touches ? (e as any).touches[0].clientY : e.clientY;
  ...
  setTool(tool: string) {
    ...
    if (tool === "select") document.getElementById("selectBtn")!.classList.add("active");
        ...
        if (tool === "triangle") document.getElementById("addTriangleBtn")!.classList.add("active");
        else if (tool === "square") document.getElementById("addSquareBtn")!.classList.add("active");
        else if (tool === "circle") document.getElementById("addCircleBtn")!.classList.add("active");
  ...
  updatePropertiesPanel(poly: Polygon) {
    ...
    (document.getElementById("warpToggle") as HTMLInputElement).checked = poly.warpMode;
    
    // Update Audio UI
    (document.getElementById("audioEnabledToggle") as HTMLInputElement).checked = poly.audioSettings.enabled;
    (document.getElementById("audioGainSlider") as HTMLInputElement).value = poly.audioSettings.gain.toString();
    (document.getElementById("audioBassSlider") as HTMLInputElement).value = poly.audioSettings.bassScale.toString();
    (document.getElementById("audioMidSlider") as HTMLInputElement).value = poly.audioSettings.midScale.toString();
    (document.getElementById("audioHighSlider") as HTMLInputElement).value = poly.audioSettings.highScale.toString();
  ...
  toggleVertexCurve() {
    if (!this.selectedPolygon || !this.selectedVertex || this.selectedVertex.type !== 'vertex') return;
  ...
  toggleFullscreen() {
    if (!document.fullscreenElement) {
      document.documentElement.requestFullscreen().catch(() => this.showStatus("Fullscreen not available"));
    } else {
  ...
  showWelcomeModal() {
    ...
    const continueBtn = document.getElementById("continueProjectBtn") as HTMLButtonElement;
  ...
  loadProjectData(data: any) {
    ...
    if (data.videos) {
      ...
      this.polygons.forEach((poly) => {
        if (poly.contentType === "video") {
          if (poly.videoSrc && this.loadedVideos.has(poly.videoSrc)) poly.loadVideo();
          else {
          ...
          if (poly.selected) {
              ...
              if (poly.warpMode && poly.gridVertices.length > 0) {
                 ...
                 poly.gridVertices.forEach((v, idx) => {
                     const x = v.x * w;
                     const y = v.y * h;
                     const isSelected = this.selectedVertex && this.selectedVertex.type === 'grid' && this.selectedVertex.index === idx;
                  ...
                  poly.vertices.forEach((v, idx) => {
                      const x = v.x * w;
                      const y = v.y * h;
                      const isSelected = this.selectedVertex && this.selectedVertex.type === 'vertex' && this.selectedVertex.index === idx;
                      ...
                      if (v.bezier) {
                          if (v.c1) {
                              ...
                              const isHandleSelected = this.selectedVertex && this.selectedVertex.type === 'c1' && this.selectedVertex.index === idx;
                              this.overlayCtx.fillStyle = isHandleSelected ? "#ff00ff" : "#ffffff";
                              this.overlayCtx.beginPath();
                          ...
                          if (v.c2) {
                              ...
                              const isHandleSelected = this.selectedVertex && this.selectedVertex.type === 'c2' && this.selectedVertex.index === idx;
                              this.overlayCtx.fillStyle = isHandleSelected ? "#ff00ff" : "#ffffff";
                              this.overlayCtx.beginPath();
                              this.overlayCtx.arc(hx, hy, 4, 0, Math.PI * 2);
                              this.overlayCtx.fill();
